...
/**
 * 获取指定用户角色对某空间级别的访问信息
 * @returns { available: boolean, reason: string } — available 是否可选, reason 锁定原因
 */
export const getLevelAccessInfo = (
  level: number,
  userRole?: string
): { available: boolean; reason: string } => {
  const levelName = SPACE_LEVEL_MAP[level] || '未知'

  if (userRole === 'admin') {
    return { available: true, reason: '' }
  }

  if (level === SPACE_LEVEL_ENUM.COMMON) {
    return { available: true, reason: '' }
  }

  if (level === SPACE_LEVEL_ENUM.PROFESSIONAL) {
    if (userRole === 'vip') {
      return { available: true, reason: '' }
    }
    return { available: false, reason: 'VIP专属' }
  }

  if (level === SPACE_LEVEL_ENUM.FLAGSHIP) {
    return { available: false, reason: '需联系管理员开通' }
  }

  return { available: false, reason: '不可用' }
}

/**
 * 生成带锁定状态的级别下拉选项（所有级别都展示，锁定项 disabled）
 */
export const getSpaceLevelOptions = (userRole?: string) => {
  return Object.keys(SPACE_LEVEL_MAP).map((key) => {
    const value = Number(key)
    const info = getLevelAccessInfo(value, userRole)
    return {
      label: info.available ? SPACE_LEVEL_MAP[value] : `${SPACE_LEVEL_MAP[value]} (${info.reason})`,
      value,
 ...
 * 获取每个级别的行动按钮文案
 */
export const getLevelActionLabel = (
  level: number,
  userRole?: string
): string => {
  const info = getLevelAccessInfo(level, userRole)
  if (info.available) {
    return '当前可用'
  }
  if (level === SPACE_LEVEL_ENUM.PROFESSIONAL && userRole === 'user') {
    return '升级VIP →'
  }
  if (level === SPACE_LEVEL_ENUM.FLAGSHIP) {
    return '联系管理员'
