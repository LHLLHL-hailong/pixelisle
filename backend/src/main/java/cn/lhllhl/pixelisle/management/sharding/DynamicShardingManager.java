package cn.lhllhl.pixelisle.management.sharding;
...
public class DynamicShardingManager {
    ...
    public void createSpacePictureTable(Space space) {
        ...
        if (space.getSpaceType() == SpaceTypeEnum.TEAM.getValue() && space.getSpaceLevel() == SpaceLevelEnum.FLAGSHIP.getValue()) {
            ...
            try {
                ...
                log.warn("创建图片空间分表失败，空间 id = {}，已降级使用主表 picture", space.getId(), e);
            }
        }
    }
    ...
    private Set<String> fetchAllPictureTableNames() {
        // 从 MySQL 获取实际存在的分表，避免查询不存在的表
        List<Map<String, Object>> tables = SqlRunner.db().selectList("SHOW TABLES LIKE 'picture_%'");
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "pixel_isle." + tableName) // 确保前缀合法
